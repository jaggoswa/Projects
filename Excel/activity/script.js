...
let allAlignBtns = document.querySelectorAll(".alignment");
let fgColor = document.querySelector("#fg-color");
...
for (let i = 0; i < Allcells.length; i++) {
    Allcells[i].addEventListener("click", function handleCell() {
        ...
        fgColor.value = cellObject.fgColor;
...
fontBtn.addEventListener("change", function () {
    ...
    let cellObject = sheetDB[rid][cid];
    cellObject.fontSize = String(fontSize);
})
...
fgColor.addEventListener("input", function(){
    ...
    let { rid, cid } = getRIdCIdfromAddress(address);
    let cell = document.querySelector(`.col[rid="${rid}"][cid="${cid}"]`);
    cell.style.color = fgColor.value;
    let cellObject = sheetDB[rid][cid];
    cellObject.fgColor = fgColor.value;
})
